...
class TwoNodeNN:
    def __init__(self, learning_rate, n_features):
        ...
        self.W2 = np.random.randn(2, 2) * 0.01             # (1, 2)
        self.b2 = np.zeros(2)                                       # scalar
    ...
    def _compute_gradients(self, x, y):
        ...
        # loss gradient wrt yhat (MSE)
        dL_dyhat = 2.0 * (yhat - y)
        dyhat_dz3 = yhat * (1.0 - yhat)
        delta3 = dL_dyhat * dyhat_dz3

        dW3 = delta3 * a2.reshape(1, -1)
        db3 = delta3
        ...
        delta1 = (self.W2.T @ delta2) * sigp_z1
    ...
    def train(self, X, y, iterations=20000, report_every=200):
        ...
        for it in range(iterations):
            i = np.random.randint(n)
            dW1, db1, dW2, db2, dW3, db3 = self._compute_gradients(X[i], y[i])
